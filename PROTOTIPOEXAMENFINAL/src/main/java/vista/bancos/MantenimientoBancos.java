...
public class MantenimientoBancos extends javax.swing.JInternalFrame {
    ...
    public void llenadoDeCombos() {
        ...
        List<banco> banco = bancoDAO.select();

        for (int i = 0; i < banco.size(); i++) {
            // Aquí podrías llenar un combo si lo necesitas
        }
    }
    ...
    public void llenadoDeTablas() {
        ...
        for (int i = 0; i < listaBancos.size(); i++) {
            ...
            dato[1] = listaBancos.get(i).getIdipo_bodega();
            dato[2] = listaBancos.get(i).getNombre();
            dato[3] = listaBancos.get(i).getDireccion();
            dato[4] = listaBancos.get(i).getStatus();
            modelo.addRow(dato);
        }
    }
    ...
    public void buscarBanco() {
        ...
        // Si tienes un txtTipoBodega:
        // txtTipoBodega.setText(bancoConsultar.getIdTipo_bodega());

        int resultadoBitacora = 0;
        Bitacora bitacoraRegistro = new Bitacora();
        resultadoBitacora = bitacoraRegistro.setIngresarBitacora(
            UsuarioConectado.getIdUsuario(), APLICACION, "Buscar Banco"
        );
    }
